// stb_audio_mixer.h v0.02 - Sean Barrett - http://nothings.org/stb/
...
typedef enum
{
   ...
   STB_FADE_linear=1,
   STB_FADE_equalpower,
   STB_FADE_release,      // faux-logarithmix curve
   STB_FADE_pulserelease, // a little volume _increase_ at the start, good for strings
} Fademode;
...
extern int stb_mixlow_present_count(void *handle);
                         ...
                         void *handle)
{
   ...
   if (stb__num_blocks == MAX_BLOCKS) { fprintf(stderr,"No free pseudo-oscillators!\n"); return; }
   ++stb__num_blocks;
...
int stb_mixlow_present_count(void *handle)
{
   int c = 0;
   int i;
   for (i=0; i < stb__num_blocks; ++i)
...
static float stb__fade(int mode, float t)
{
   ...
   switch (mode) {
      ...
      case STB_FADE_pulserelease: 
                            ...
                            return p*(r < d ? d : r);
      case STB_FADE_release:    r = t*t*t; r=r*r; r *= 0.5f; // pseudo log
                            d = 1-(1-t)*15;
                            return r < d ? d : r;
      case STB_FADE_none:
